...
public class SubMixer : ISampleSource, IDisposable
{
    ...
    public SubMixer(int sampleRate, int bits, int channelCount, AudioEncoding encoding)
    {
        if (channelCount < 1)
        ...
        _waveFormat = new WaveFormat(sampleRate, bits, channelCount, encoding);
        FillWithZeros = false;
    }
